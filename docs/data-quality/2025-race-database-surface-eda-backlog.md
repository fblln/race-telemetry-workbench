...
- [x] Filter all primary queries to `year = 2025` and `session_type = 'R'`.
- [x] Keep 2024/2026 out of plots, summaries, and conclusions unless explicitly
  shown in a clearly labeled comparison appendix.
- [x] Preserve race-default behavior. Do not include FP, Q, SQ, S, FP1, FP2, or
  FP3 unless a future task explicitly requests them.
- [x] Make the notebook title, summary, artifact directory, and metadata say
  `2025 race sessions`.
- [x] Add a guardrail assertion that exactly 24 2025 race sessions are present
  before season-level conclusions are generated.

## Iteration 1 - Reframe The Surface Quality Model

- [x] Split surface QA into product-readiness lenses:
...
- [x] Add labels:
  ...
  - `needs_reimport`
  - `needs_manual_review`
- [x] Add severity scores instead of relying only on booleans:
  - affected drivers
  - affected laps or windows
  - affected rows
  - percent of session time affected
  - product impact: launcher, replay, lap comparison, context panels
- [x] Separate systematic known limitations from surprising defects. Example:
...
- [x] Aggregate aligned quality by race, driver, lap, and session-time window.
- [x] Decode `quality_flags` into stable families:
  - `OK`
  - car gap too large
  - car sample too old
...
- [x] Distinguish isolated stale rows from long degraded bursts.
- [x] Join aligned quality to lap metadata and context:
  - pit in/out
  - SC/VSC/red flag
  - missing lap time
  - FastF1 inaccurate
- [x] Add representative replay-quality strips:
  - one row per driver
  - x-axis session time
  - color by aligned quality family
  - markers for SC/VSC/red flag and pit windows where available
- [x] Identify races/drivers where replay is likely to appear jumpy, stale, or
  incomplete.
...
- [x] Check weather sample cadence by race.
- [x] Flag large weather sampling gaps.
- [x] Detect suspicious jumps in:
  - air temperature
  - track temperature
  - pressure
  - humidity
  - wind direction/speed
- [x] Identify rainfall transitions and align them with track status and race
  control.
- [x] Add per-race weather trend panels for races with rainfall or large value
  shifts.

### Track And Session Status

- [x] Convert status events into intervals with derived duration.
- [x] Compare status intervals against race-control messages.
- [ ] Detect sessions with status timelines that begin late, end early, or have
  ambiguous transitions.
- [x] Add status timeline strips for every 2025 race.

### Race Control

- [x] Build a race-control message taxonomy:
  - flags
  - safety car
  - VSC
  - DRS
  ...
  - pit entry/exit
  - other
- [x] Add duplicate and near-duplicate detection.
- [x] Cluster message text using a skrub/scikit-learn-compatible text pipeline.
- [x] Quantify missing timing, missing lap scope, and missing driver scope by
  category.
- [x] Add incident density timelines by race.
- [x] Preserve example messages for each taxonomy bucket.

## Iteration 5 - Circuit Marker And Track Context QA

- [x] Audit circuit metadata and markers for all 24 2025 races:
...
- [x] Compare marker coordinates with imported position trace bounds.
- [x] Flag markers outside plausible position bounds.
- [x] Add marker-over-track visual examples for selected circuits.
...
- [x] Derive `session_duration_ms` from:
...
- [x] Use derived duration to measure surface coverage:
  - weather coverage ratio
  - status coverage ratio
  - race-control time span
  - raw telemetry span
  - aligned telemetry span
- [x] Treat missing `session_end_utc` as a known limitation, then show whether
  derived duration is sufficient for replay/context QA.
- [x] Add a coverage-window chart per race showing which surfaces cover which
...
- [x] Add aligned-quality driver/race heatmap.
- [x] Add race-control/status timeline strips.
- [x] Add weather cadence and jump plots.
- [x] Add circuit-marker sanity panels.
- [x] Add a final recommendation table: no action, label in UI, inspect, reimport,
...
- [ ] Add a CLI for the surface EDA support module:
  ...
  - `--refresh-cache`
  - `--skip-skrub-report`
- [x] Cache expensive aggregates as Parquet.
- [x] Write metadata JSON with:
  - year
  - session type
  - session count
  - table row counts
...
- [x] Add a clear command in the notebook and docs for rerunning the full EDA.
...
1. [x] Refactor the current broad surface notebook into a 2025-only notebook and
   artifact directory.
2. [x] Build aligned replay quality aggregation by race, driver, lap, and time
   window.
3. [x] Add aligned-quality timeline strips and driver/race heatmaps.
4. [x] Derive session duration and coverage windows from imported samples/events.
5. [x] Add race-control taxonomy and timeline density.
6. [x] Add weather cadence/jump checks.
7. [x] Add circuit-marker sanity visuals.
