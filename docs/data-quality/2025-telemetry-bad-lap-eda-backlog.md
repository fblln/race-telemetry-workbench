...
- [x] Split the current single taxonomy into three independent lenses:
...
- [x] Rename or document `shape_mismatch_against_comparable_laps` as
  ...
  defect.
- [x] Add severity scores next to boolean flags:
  - coverage deficit
  ...
  - null-rate severity
- [x] Make threshold configuration visible in the notebook and markdown summary,
  including rationale and expected failure mode.

## Iteration 2 - Improve Category Overlap And Attribution

- [x] Add an UpSet-style category-intersection table or plot. Current category
  bars are useful but hide overlap.
- [x] Add a decision waterfall:
  - total laps
  ...
  - shape-only outliers
  - clean
- [x] Add per-race stacked category decomposition so high bad-lap rates explain
  themselves immediately.
- [x] Add a primary-category audit table with examples where the primary
  category may hide a more interesting secondary reason.
- [x] Add `reason_count` and `reason_set` columns to the classified-lap table for
  easier filtering and grouping.

## Iteration 3 - Stronger Visual Storytelling

- [x] Add a lap-number heatmap:
  - rows: race
  - columns: lap number
  - color: primary quality class
  - overlay or mark SC/VSC/red-flag periods where available
- [x] Add a driver-by-race matrix for bad-lap rate with minimum-lap-count
  filtering and separate views for integrity-only and context-only flags.
- [x] Replace or complement the race bad-rate bar with stacked bars showing the
  dominant causes per race.
- [x] Add selected race drilldowns for British, Belgian, Australian, Dutch, and
...
- [x] Add a final recommendation figure: keep, keep-with-context-label, exclude,
  manual-review.

## Iteration 4 - Better Shape Analysis

- [x] Compare each lap against a clean same-race median speed profile with
  quantile bands, not only a residual score.
- [x] For representative traces, show:
  - clean median profile
  - 10th-90th percentile band
  - selected anomalous lap
  - contextual markers for pit/SC/VSC/red flag where available
- [x] Stratify speed-shape baselines by at least one of:
  - compound
  - stint number
...
- [x] Add cluster stability checks across `k` values and random seeds before
...
- [x] Add threshold sensitivity sweeps for:
  - minimum car samples
...
- [x] Show which races/drivers/categories are stable versus threshold-sensitive.
- [ ] Add bootstrap or resampling checks for speed-profile baseline bands.
- [x] Record the selected threshold set in a machine-readable artifact.
- [x] Add a "borderline laps" table for manual review.
...
- [x] Save classified laps as Parquet as well as CSV for type preservation.
- [x] Add a JSON metadata file with:
...
- [x] Keep artifact caches ignored while preserving figures, tables, markdown,
  and the `skrub` report.

## Iteration 9 - Tests And Engineering Hardening

- [x] Add unit tests for classification rules using small synthetic lap rows.
- [x] Add tests that verify mutually exclusive and non-mutually-exclusive
  behavior:
...
1. [x] Add category overlap, waterfall, and stacked race-decomposition visuals.
2. [x] Rename and reframe speed-shape mismatch as atypical speed profile.
3. [x] Add race drilldowns for British, Belgian, and Australian.
4. Persist/join per-lap Apexline diagnostics.
5. [x] Add threshold sensitivity and borderline-lap review.
