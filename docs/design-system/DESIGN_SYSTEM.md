...
**Race story (natural language, MCP).** The AI Assistant Panel (§8.10). The MCP server chains telemetry, stint, and event queries and an LLM composes a narrative briefing rather than charts. Rendered as a conversational surface: the user's question as an amber-muted bubble (right-aligned), the answer as **plain prose — not boxed** — at 15px with key facts emphasized, and the tool calls **collapsed to a quiet "Used N sources" line** beneath so the reasoning stays auditable without console noise. The composer is a rounded pill with an auto-grow textarea and a circular send button (Enter sends, Shift+Enter newlines). Always answered from bounded analytical endpoints, never raw sample dumps.
